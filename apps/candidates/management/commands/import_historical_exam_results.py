"""
Management Command: import_historical_exam_results
===================================================
فاز 3-1 ایمپورت تاریخی — ثبت نتایج آزمون کتبی متقاضیان

استفاده:
    python manage.py import_historical_exam_results <path_to_excel>
                       [--sheet کتبی]
                       [--dry-run]
"""
...
class Command(BaseCommand):
    help = 'فاز 3-1 ایمپورت تاریخی: ثبت نتایج آزمون کتبی متقاضیان از فایل اکسل'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help='نام شیت (پیش‌فرض: جستجوی خودکار شیت کتبی)'
        )
        parser.add_argument(
            '--dry-run',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.HTTP_INFO('  ایمپورت نتایج آزمون کتبی متقاضیان — فاز 3-1'))
        if dry_run:
        ...
        if sheet_name_arg:
            ...
            # جستجوی خودکار شیت حاوی کلمه کتبی (ترجیحاً اونی که با عدد تموم میشه یا کتبی خالی)
            for name in sheet_names:
                if 'کتبی' in name:
                    main_sheet = name
                    # اگر کتبی۲ وجود داشت، کتبی۲ را ترجیح بده چون حاوی داده‌های کامل‌تر است
                    if 'کتبی2' in name:
                        break
            if not main_sheet:
                raise CommandError("شیت حاوی نتایج آزمون 'کتبی' یافت نشد.")
        ...
        col_map['score'] = find_col_idx(headers, ['scorew', 'score_w', 'نمره آزمون کتبی', 'نمره آزمون', 'نمره'])
        col_map['result'] = find_col_idx(headers, ['result1', 'result', 'وضعیت قبولی آزمون', 'وضعیت قبولی', 'وضعیت'])

        self.stdout.write('نگاشت ستون‌ها:')
        field_labels = {
            'exam_code': 'شماره آزمون (کد شغل)',
            'national_id': 'کد ملی متقاضی',
            'score': 'نمره آزمون کتبی',
            'result': 'وضعیت نتیجه آزمون کتبی',
        }
        ...
        if any(col_map[f] is None for f in ['exam_code', 'national_id', 'score']):
            raise CommandError('برخی از ستون‌های حیاتی آزمون کتبی (کد ملی، شماره آزمون یا نمره کتبی) یافت نشد.')
    ...
    def _run_import(self, data_rows, col_map, dry_run):
        ...
        # Cache برای مراحل آزمون کتبی فرصت‌های شغلی جهت سرعت بیشتر
        stage_cache = {}
        ...
        for row_num, row in enumerate(data_rows, start=2):
            ...
            if not app:
                stats['warnings'].append(
                    f'ردیف {row_num}: درخواست همکاری برای متقاضی {national_id} در آزمون "{exam_code}" یافت نشد — رد شد'
                )
                continue

            job = app.job

            # پیدا کردن مرحله آزمون کتبی مربوط به این شغل
            if job.id not in stage_cache:
                # اولویت: مرحله‌ای از نوع EXAM که کلمه «کتبی» در نامش باشد و شامل «مهارتی» نباشد
                exam_stage = job.stages.filter(stage_type='EXAM', is_deleted=False).filter(
                    Q(name__icontains='کتبی') | ~Q(name__icontains='مهارتی')
                ).first()
                if not exam_stage:
                    # در غیر این صورت، اولین مرحله از نوع EXAM
                    exam_stage = job.stages.filter(stage_type='EXAM', is_deleted=False).first()
                stage_cache[job.id] = exam_stage

            stage = stage_cache[job.id]

            if not stage:
                stats['warnings'].append(
                    f'ردیف {row_num}: هیچ مرحله ارزیابی از نوع "آزمون" (EXAM) برای فرصت شغلی {exam_code} یافت نشد — رد شد'
                )
                continue
        ...
        self.stdout.write('')
        self.stdout.write(self.style.HTTP_INFO('═' * 60))
        self.stdout.write(self.style.HTTP_INFO('  گزارش نهایی ایمپورت نتایج آزمون کتبی'))
        self.stdout.write(self.style.HTTP_INFO('═' * 60))
        ...
        if dry_run:
            ...
            self.stdout.write(self.style.SUCCESS('ایمپورت نتایج آزمون کتبی با موفقیت به پایان رسید.'))
        self.stdout.write('')
