"""
Management Command: import_historical_interview_results
======================================================
فاز 3-3 ایمپورت تاریخی — ثبت نتایج مصاحبه متقاضیان

استفاده:
    python manage.py import_historical_interview_results <path_to_excel>
                       [--sheet مصاحبه4]
                       [--dry-run]
"""
...
class Command(BaseCommand):
    help = 'فاز 3-3 ایمپورت تاریخی: ثبت نتایج مصاحبه متقاضیان از فایل اکسل'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help='نام شیت (پیش‌فرض: جستجوی خودکار شیت مصاحبه)'
        )
        parser.add_argument(
            '--dry-run',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.HTTP_INFO('  ایمپورت نتایج مصاحبه متقاضیان — فاز 3-3'))
        if dry_run:
        ...
        if sheet_name_arg:
            ...
            # جستجوی خودکار شیت حاوی کلمه مصاحبه
            for name in sheet_names:
                if 'مصاحبه' in name:
                    main_sheet = name
                    # اگر مصاحبه۴ وجود داشت، مصاحبه۴ را ترجیح بده چون حاوی داده‌های کامل‌تر است
                    if 'مصاحبه4' in name:
                        break
            if not main_sheet:
                raise CommandError("شیت حاوی نتایج آزمون 'مصاحبه' یافت نشد.")
        ...
        col_map['score'] = find_col_idx(headers, ['scorei', 'score_i', 'نمره مصاحبه', 'نمره'])
        col_map['cscore'] = find_col_idx(headers, ['cscorei', 'cscore_i', 'امتیاز تجمعی', 'نمره تجمعی'])
        col_map['result'] = find_col_idx(headers, ['result3', 'result', 'وضعیت قبولی مصاحبه', 'وضعیت مصاحبه', 'وضعیت قبولی', 'وضعیت'])

        self.stdout.write('نگاشت ستون‌ها:')
        field_labels = {
            'exam_code': 'شماره آزمون (کد شغل)',
            'national_id': 'کد ملی متقاضی',
            'score': 'نمره مصاحبه (ScoreI)',
            'cscore': 'امتیاز تجمعی (CScoreI)',
            'result': 'وضعیت نتیجه مصاحبه (Result3)',
        }
        ...
        if any(col_map[f] is None for f in ['exam_code', 'national_id', 'score']):
            raise CommandError('برخی از ستون‌های حیاتی مصاحبه (کد ملی، شماره آزمون یا نمره مصاحبه) یافت نشد.')
    ...
    def _run_import(self, data_rows, col_map, dry_run):
        ...
        # Cache برای مراحل مصاحبه فرصت‌های شغلی جهت سرعت بیشتر
        stage_cache = {}
        ...
        for row_num, row in enumerate(data_rows, start=2):
            ...
            # پیدا کردن مرحله مصاحبه مربوط به این شغل
            if job.id not in stage_cache:
                # مرحله‌ای از نوع INTERVIEW
                interview_stage = job.stages.filter(stage_type='INTERVIEW', is_deleted=False).first()
                stage_cache[job.id] = interview_stage

            stage = stage_cache[job.id]

            if not stage:
                stats['warnings'].append(
                    f'ردیف {row_num}: هیچ مرحله ارزیابی از نوع "مصاحبه" (INTERVIEW) برای فرصت شغلی {exam_code} یافت نشد — رد شد'
                )
                continue
        ...
        self.stdout.write('')
        self.stdout.write(self.style.HTTP_INFO('═' * 60))
        self.stdout.write(self.style.HTTP_INFO('  گزارش نهایی ایمپورت نتایج مصاحبه'))
        self.stdout.write(self.style.HTTP_INFO('═' * 60))
        ...
        if dry_run:
            ...
            self.stdout.write(self.style.SUCCESS('ایمپورت نتایج مصاحبه با موفقیت به پایان رسید.'))
        self.stdout.write('')
