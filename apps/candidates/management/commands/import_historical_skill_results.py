"""
Management Command: import_historical_skill_results
===================================================
فاز 3-2 ایمپورت تاریخی — ثبت نتایج آزمون مهارتی متقاضیان

استفاده:
    python manage.py import_historical_skill_results <path_to_excel>
                       [--sheet مهارتی3]
                       [--dry-run]
"""
...
class Command(BaseCommand):
    help = 'فاز 3-2 ایمپورت تاریخی: ثبت نتایج آزمون مهارتی متقاضیان از فایل اکسل'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help='نام شیت (پیش‌فرض: جستجوی خودکار شیت مهارتی)'
        )
        parser.add_argument(
            '--dry-run',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.HTTP_INFO('  ایمپورت نتایج آزمون مهارتی متقاضیان — فاز 3-2'))
        if dry_run:
        ...
        if sheet_name_arg:
            ...
            # جستجوی خودکار شیت حاوی کلمه مهارتی
            for name in sheet_names:
                if 'مهارتی' in name:
                    main_sheet = name
                    # اگر مهارتی۳ وجود داشت، مهارتی۳ را ترجیح بده چون حاوی داده‌های کامل‌تر است
                    if 'مهارتی3' in name:
                        break
            if not main_sheet:
                raise CommandError("شیت حاوی نتایج آزمون 'مهارتی' یافت نشد.")
        ...
        col_map['score'] = find_col_idx(headers, ['scores', 'score_s', 'نمره آزمون مهارتی', 'نمره مهارتی', 'نمره عملی', 'نمره'])
        col_map['result'] = find_col_idx(headers, ['result2', 'result', 'وضعیت قبولی آزمون مهارتی', 'وضعیت مهارتی', 'وضعیت قبولی مهارتی', 'وضعیت'])

        self.stdout.write('نگاشت ستون‌ها:')
        field_labels = {
            'exam_code': 'شماره آزمون (کد شغل)',
            'national_id': 'کد ملی متقاضی',
            'score': 'نمره آزمون مهارتی',
            'result': 'وضعیت نتیجه آزمون مهارتی',
        }
        ...
        if any(col_map[f] is None for f in ['exam_code', 'national_id', 'score']):
            raise CommandError('برخی از ستون‌های حیاتی آزمون مهارتی (کد ملی، شماره آزمون یا نمره مهارتی) یافت نشد.')
    ...
    def _run_import(self, data_rows, col_map, dry_run):
        ...
        # Cache برای مراحل آزمون مهارتی فرصت‌های شغلی جهت سرعت بیشتر
        stage_cache = {}
        ...
        for row_num, row in enumerate(data_rows, start=2):
            ...
            # پیدا کردن مرحله آزمون مهارتی مربوط به این شغل
            if job.id not in stage_cache:
                # مرحله‌ای از نوع SKILL_TEST
                skill_stage = job.stages.filter(stage_type='SKILL_TEST', is_deleted=False).first()
                stage_cache[job.id] = skill_stage

            stage = stage_cache[job.id]

            if not stage:
                stats['warnings'].append(
                    f'ردیف {row_num}: هیچ مرحله ارزیابی از نوع "آزمون مهارتی" (SKILL_TEST) برای فرصت شغلی {exam_code} یافت نشد — رد شد'
                )
                continue

            # تعیین وضعیت مرحله ارزیابی
            # ۱. غایب -> FAILED و یادداشت غایب و نمره صفر
            # ۲. مجاز -> COMPLETED
            # ۳. غیر مجاز -> FAILED
            ...
            if 'غایب' in result_str:
                ...
                notes_str = 'غایب در آزمون مهارتی'
                score_val = 0.0
        ...
        self.stdout.write('')
        self.stdout.write(self.style.HTTP_INFO('═' * 60))
        self.stdout.write(self.style.HTTP_INFO('  گزارش نهایی ایمپورت نتایج آزمون مهارتی'))
        self.stdout.write(self.style.HTTP_INFO('═' * 60))
        ...
        if dry_run:
            ...
            self.stdout.write(self.style.SUCCESS('ایمپورت نتایج آزمون مهارتی با موفقیت به پایان رسید.'))
        self.stdout.write('')
